...
class Color:
    ...
    def get_color_percentage(self, image: np.array) -> float:
        image_filtered = self.filter_image(image)
        color_mask = cv2.inRange(frame_hsv, self.lower_bound, self.upper_bound)

                # Calculate the total number of pixels in the image
        total_pixels = image_filtered.size // image_filtered.itemsize
...
def color_search() -> None:
    ...
    cv2.setMouseCallback("Color finder", mouseClick, param = None)
